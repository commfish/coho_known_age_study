...
# This analysis was originally to be done using QDA. To perform this, we checked for normality of the predictor variables. 
# Even though the model performed well with QDA, because we could not ensure normality, we decided to use binomial regression.
# Below is the QDA analysis, for variables Q2 / Q2plus and Q9abs. 
...
# Run QDA
fit_qda_final <- qda(as.factor(Age) ~ Location + Q2 + Q9abs, data=coho_scales_bothriv,
...
predictedvals <- expand.grid(Q9abs =seq(min(coho_scales_bothriv$Q9abs), 
                             ...
                             Q2=seq(min(coho_scales_bothriv$Q2), 
                                    max(coho_scales_bothriv$Q2)+0.1, length.out = 100), #adding a little bit for plotting later
                             Location = c("HS", "BR"))
...
#write.csv(write.csv(final, "data/check_611.csv"))
...
ggplot() + 
  ...
  ggtitle("A) Berners River")+ xlab ("Distance: Circulus 3 - Circulus 8")+ylab("Distance: Circulus 3 - Scale Edge") +
...
ggplot() + 
  ...
  scale_y_continuous(expand = c(0,0), limits = c(0.09, 0.72), breaks = c(seq(0.1, 0.7, by = 0.1))) + 
  scale_x_continuous(expand = c(0,0), limits = c(0.07, 0.23), breaks = c(seq(0.08, 0.2, by = 0.04))) +
  ggtitle("B) Hugh Smith")+ xlab ("Distance: Circulus 3 - Circulus 8") + ylab("Circulus 3 - Scale Edge") +
  annotate("text", x = 0.19, y=0.13, label="predicted age 1", family="Times New Roman", colour="black", size=3) +
  annotate("text", x = 0.1, y=0.68, label="predicted age 2", family="Times New Roman", colour="black", size=3) +
